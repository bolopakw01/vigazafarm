...
<style>
	...
	/* wider container specifically for split layout to center left/right */
	...
	/* animated subtle background for the split card — Vigaza Farm theme (eggshell / puyuh), slightly darker */
	...
	/* slightly stronger 'speckle' to make background feel a bit darker/textured */
	...
	@keyframes splitBgMove{
		0%{background-position:0% 50%}
		50%{background-position:100% 50%}
		100%{background-position:0% 50%}
	}

	/* ensure children are above animated background */
	...
	.big404{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-weight:900;color:#ffffff;opacity:0.12;font-size:clamp(12rem,18vw,32rem);letter-spacing:0.02em;z-index:1;pointer-events:none;text-shadow:0 2px 8px rgba(0,0,0,0.45);font-family:Anton, sans-serif;text-transform:uppercase}
	...
	@media (max-width:480px){.card{height:300px}.overlay h1{font-size:1.25rem}.overlay p{font-size:1rem}.big404{font-size:8rem;opacity:0.12}}

	/* Small-screen: colorful no-signal effect */
	@media (max-width:480px){
		.noise_bg{background: linear-gradient(
			to right,
			#002fc6 0%,
			#002bb2 14.2857%,
			#3a3a3a 14.2857%,
			...
			white 85.7143%,
			#fafafa 100%
		);background-size:100% 100%;animation:none;filter:none;opacity:1;mix-blend-mode:normal;border-radius:10px;border:2px solid black;display:flex;align-items:center;justify-content:center;overflow:hidden;z-index:0;color:#252525;font-weight:bold;letter-spacing:.15em}

		.noise_bg::before{content:"";position:absolute;left:0;top:0;width:100%;height:68.47826%;background:linear-gradient(
			to right,
			white 0%,
			#fafafa 14.2857%,
			#ffe60a 14.2857%,
			...
			#002fc6 85.7143%,
			#002bb2 100%
		);z-index:1;pointer-events:none;mix-blend-mode:overlay;opacity:0.95}

		.noise_bg::after{content:"";position:absolute;left:0;bottom:0;width:100%;height:21.73913%;background:linear-gradient(
			to right,
			#006c6b 0%,
			#005857 16.6667%,
			white 16.6667%,
			...
			#3a3a3a 83.3333%,
			#303030 100%
		);z-index:1;pointer-events:none;mix-blend-mode:overlay;opacity:0.9}
...
<style>
	/* No-signal state (applies after noise animation ends) */
	.noise_bg.no-signal{
		...
		opacity:1 !important;
		transition:background .6s ease, opacity .6s ease, filter .6s ease;
	}

	.noise_bg.no-signal::before{
		content:"";position:absolute;left:0;top:0;width:100%;height:68.47826%;background:linear-gradient(
			to right,
			white 0%,
			#fafafa 14.2857%,
			#ffe60a 14.2857%,
	...
	.noise_bg.no-signal::after{
		content:"";position:absolute;left:0;bottom:0;width:100%;height:21.73913%;background:linear-gradient(
			to right,
			#006c6b 0%,
			#005857 16.6667%,
			white 16.6667%,
...
@section('content')
  <div class="wrap split-wrap">
    <div class="split-card" role="group" aria-label="404 split card">
      <div class="split-left">
        <div class="card" role="region" aria-label="Not Found 404">
      ...
      <div class="split-right" role="region" aria-label="404 info panel">
				<div class="right-content">
					...
					<p>Waduh Pak! — Sepertinya halaman ini belum bertelur di kandang kami. Mungkin alamatnya keliru, halaman sudah dipindahkan, atau belum dibuat. Silakan periksa kembali URL, atau kembali ke halaman Utama untuk melanjutkan.</p>
					<div class="actions">
	...
	<script>
		// Play animated noise for a few seconds, then switch to "no-signal" gradient
		(function(){
			const DELAY_MS = 3500; // milliseconds to play noise
			document.addEventListener('DOMContentLoaded', function(){
				const noise = document.querySelector('.noise_bg');
				if(!noise) return;

				// optional initial flicker (subtle)
				noise.style.transition = 'filter 400ms ease, opacity 500ms ease, background-position 400ms linear';

				setTimeout(() => {
					// switch to no-signal look
