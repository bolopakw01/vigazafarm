  ...
  <div class="col-12">
    ...
    <select id="pembesaran_id" name="pembesaran_id" class="form-select @error('pembesaran_id') is-invalid @enderror">
      ...
      @foreach($pembesaranList as $pembesaran)
        @php
          $stokAwal = $pembesaran->jumlah_siap ?: $pembesaran->jumlah_anak_ayam ?: 0;
          $stokTersedia = max((int) $stokAwal - (int) ($pembesaran->indukan_ditransfer ?? 0), 0);
        @endphp
        <option value="{{ $pembesaran->id }}"
            ...
            {{ $pembesaran->batch_label }} - {{ $pembesaran->tanggal_siap ? $pembesaran->tanggal_siap->format('d/m/Y') : 'TBA' }} - Stok: {{ $stokTersedia }} ekor (Selesai)
        </option>
      @endforeach
    </select>
    @error('pembesaran_id')
