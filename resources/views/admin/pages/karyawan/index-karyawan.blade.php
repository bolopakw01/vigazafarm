...
<script>
	...
	const tableUtils = (() => {
		...
		const collectHeaders = (table) => {
			...
			table.querySelectorAll('thead th').forEach((th, index) => {
				...
				if (!text || ['aksi', 'foto', 'harga'].includes(text.toLowerCase())) {
	...
	(function setupExportAndPrint() {
		...
		exportBtn?.addEventListener('click', () => {
			...
			tableUtils.downloadCSV(csv, `karyawan-${tableUtils.timestamp()}.csv`);
			showToast('File CSV berhasil disiapkan', 'success');
		});

		printBtn?.addEventListener('click', () => {
			const rows = tableUtils.collectRows(table, skipIndexes);
			tableUtils.printTable('Data Karyawan', headers, rows);
		});
	})();
